...
// 조건 값 변수
	...
	$fp	= "/usr/local/apache/htdocs/project_os/DB/gpio_config_DB/";
	$fp	.= $number;
	...
	{
		echo "abcdefg";
		$fp = "/usr/local/apache/htdocs/project_os/DB/gpio_analog/";
		$fp .= $number;
	...
	if($pair != 0)
	{
		$fp	= "/usr/local/apache/htdocs/project_os/DB/gpio_pair/";
		$fp	.= $number;
	...
	if($pwm != 0)
	{
		$fp = "/usr/local/apache/htdocs/project_os/DB/gpio/";
		$fp .= $number;
	...
	if($pwm_condition != 0)
	{
		$fp = "/usr/local/apache/htdocs/project_os/DB/gpio_pwm/";
		$fp .= $number;
	...
	for($j = 0 ; $j < $compareNumberSize ; $j++)
	{
		$fp				= "/usr/local/apache/htdocs/project_os/DB/gpio_compare/";
		$fp				.= $number;
	...
	if($gpio_variable != NULL && $variable_area != NULL && $gpio_variable_index != 0)
	{
		...
		$fp			= "/usr/local/apache/htdocs/project_os/DB/gpio_variable/";
		$fp			.= $gpio_variable;
		unlink($fp);
